...
const parseLine = (line) => {
  ...
  let rule = splitLine[0];
  let checkLetter = rule.split(' ')[1];
  let limits = rule.split(' ')[0].split('-');
  let passwordSplit = password.slice(1, ).split('');
  let conditionFlag = null;
  console.log(
    //'checkLetter', checkLetter,
  //'passwordSplit', passwordSplit,
  'password', password,
  'passwordSplit[lower limit]', passwordSplit[limits[0] - 1],
  'passwordSplit[upper limit]', passwordSplit[limits[1] - 1],
  'limit[0]', limits[0],
  'limit[1]', limits[1]
  );

  if(passwordSplit[limits[0] - 1] === checkLetter) {
    console.log('checking if 1st letter matches', passwordSplit[limits[0] - 1], checkLetter);
  }

  if(passwordSplit[limits[1] - 1] === checkLetter) {
    console.log('checking if second letter matches', passwordSplit[limits[1] - 1], checkLetter);
  }

  if(passwordSplit[limits[0] - 1] === checkLetter && passwordSplit[limits[1] - 1] != checkLetter) {
    console.log('checking if 1st letter matches, and 2nd doesnt', passwordSplit[limits[0] - 1], checkLetter);
  }

  if(passwordSplit[limits[1] - 1] === checkLetter && passwordSplit[limits[0] - 1] != checkLetter) {
    console.log('checking if second letter matches, but not first');
  }
  
  console.log('do we meet the condition', (passwordSplit[limits[0] - 1] === checkLetter && passwordSplit[limits[1] - 1] != checkLetter)
    || (passwordSplit[limits[1] - 1] === checkLetter && passwordSplit[limits[0] - 1] != checkLetter));

  return (passwordSplit[limits[0] - 1] === checkLetter && passwordSplit[limits[1] - 1] != checkLetter)
    || (passwordSplit[limits[1] - 1] === checkLetter && passwordSplit[limits[0] - 1] != checkLetter);
}
...
for(let j = 1; j < dataSplit.length - 1; j++) {
  console.log('j', j, dataSplit[j]);
  if (parseLine(dataSplit[j])) {
    totalGood++;
  }
}
