let data7Raw = `mirrored beige bags contain 1 drab brown bag, 3 dotted crimson bags.
...
let sampleData7Raw = `light red bags contain 1 bright white bag, 2 muted yellow bags.
...
//More Formatted Data Structure Initialization
let colorDictionary = [];
let nodeDictionary = {};
let goldBagCount = 0;
...
data7Split.forEach((line) => {
  line7Split.push(line.split(' bag'))
});

line7Split.forEach((line, lineIndex) => {
  // console.log('line', line, 'index', lineIndex);
  colorDictionary.push(line[0]);
  line.forEach((color) => {
    if(color === 's.' || color === '.') {
      // console.log('hi', typeof line7Split[lineIndex][colorIndex]);
...
const getColor = (lineItem) => {
  let result;
  colorDictionary.forEach((colorInDict) => {
    // console.log('in getColor', colorInDict);
    if (lineItem.includes(colorInDict)) {
      result = colorInDict;
    }
  })
  return result;
...
line7Split.forEach((line) => {
  createNewNodeFromLine(line);
})

const parentFillIn = (arrOfValues) => {
  arrOfValues.forEach((node) => {
    // console.log('in parentFillIn', nodeDictionary[node]);
    if (nodeDictionary[node].children) {
      nodeDictionary[node].children.forEach(child => {
        if(!nodeDictionary[child].parent) {
          nodeDictionary[child].parent = nodeDictionary[node].value
        }
      })
    }
  });
...
parentFillIn(colorDictionary);

// console.log(nodeDictionary);

let nodeGoldBagCounter = 0;
const checkBagContainsGold = (theNode) => {
  // console.log('nodeGoldBagCounter', nodeGoldBagCounter);
  const recursiveChecker = (node) => {
  ...
  recursiveChecker(theNode);

  return nodeGoldBagCounter > 0;


  // haveWeCountedYetArr.push(node.value);
  //   if(node.children.length < 1) {
  //     // console.log('nothing to do here');
  //     return 1 === 0;
  //   } 
  //   else {
  //     console.log(node.value, node.children.includes('shiny gold'), node.children);
  //     if(node.children.includes('shiny gold')) {
  //       childrenContainGold = true;
  //       return node.children.includes('shiny gold');
  //     } else {
  //       node.children.forEach(child => {
  //         return checkBagContainsGold(nodeDictionary[child] || childrenContainGold);
  //       });
  //     }
  //   }
}
